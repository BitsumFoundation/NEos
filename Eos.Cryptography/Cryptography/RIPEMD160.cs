...
namespace NEos.Cryptography
{
    public static class RIPEMD160
    {
        ...
        public static byte[] ComputeHash(byte[] buffer, int offset, int length)
        {
            ...
            var buf = new UInt32[(length + addLen) / 4];

            Buffer.BlockCopy(buffer, offset, buf, 0, length);
            buf[length / 4] |= (uint)1 << 8 * (length % 4) + 7;

            buf[buf.Length - 2] = (uint)length << 3;
            buf[buf.Length - 1] = (uint)length >> 29;

            return ComputeHash(buf);
        }

        private static byte[] ComputeHash(uint[] x)
        {
            var h0 = H0;
            var h1 = H1;
            var h2 = H2;
